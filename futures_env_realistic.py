    ...
    def __init__(self, 
        ...
        if len(self.states) > 0:
            ...
            if self.add_current_position_to_state:
                ...
                # Add space for risk metrics (3 additional features when in position)
                # We'll use the max possible size for the observation space
                state_shape = (state_shape[0] + 3,)
            ...
            default_shape = (60 + 1 + 3,) if self.add_current_position_to_state else (60,)
            self.observation_space = spaces.Box(
