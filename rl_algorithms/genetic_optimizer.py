...
class GeneticOptimizer:
    ...
    def has_converged(self, best_fitness_history):
        if len(best_fitness_history) < self.convergence_generations:
            return False
        recent = best_fitness_history[-self.convergence_generations:]
    ...
    def evolve(self, base_params: Dict[str, float], performance_score: float) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            for generation in range(min(10, self.max_generations)):  # Limit generations per call
                ...
                if self.has_converged(self.best_fitness_history):
                    logger.info(f"Genetic algorithm converged at generation {self.generation}")
                    break

            # Return best parameters if improvement found
            if self.best_individual and self.best_individual.fitness > performance_score:
                logger.info(
                    f"Genetic optimization improved fitness from {performance_score:.4f} to {self.best_individual.fitness:.4f}")
